...
設定ファイルパス: `~/.ebiclaw/config.json`

### 環境変数

環境変数を使用してデフォルトパスを上書きできます。ポータブルインストール、コンテナ化デプロイ、または ebiclaw をシステムサービスとして実行する場合に便利です。これらの変数は独立しており、異なるパスを制御します。

| 変数              | 説明                                                                                                                             | デフォルトパス            |
|-------------------|-----------------------------------------------------------------------------------------------------------------------------------------|---------------------------|
| `EBICLAW_CONFIG` | 設定ファイルのパスを上書きします。ebiclaw がどの `config.json` を読み込むかを直接指定し、他のすべての場所を無視します。 | `~/.ebiclaw/config.json` |
| `EBICLAW_HOME`   | ebiclaw データのルートディレクトリを上書きします。`workspace` やその他のデータディレクトリのデフォルト場所を変更します。          | `~/.ebiclaw`             |

**例：**

```bash
# 特定の設定ファイルで ebiclaw を実行
# ワークスペースパスはその設定ファイル内から読み込まれます
EBICLAW_CONFIG=/etc/ebiclaw/production.json ebiclaw gateway

# /opt/ebiclaw にすべてのデータを保存して ebiclaw を実行
# 設定はデフォルトの ~/.ebiclaw/config.json から読み込まれます
# ワークスペースは /opt/ebiclaw/workspace に作成されます
...
環境変数でも上書き可能です：`EBICLAW_LOG_LEVEL=info`

### ワークスペースレイアウト

EbiClaw は設定されたワークスペース（デフォルト: `~/.ebiclaw/workspace`）にデータを保存します：
...
1. `~/.ebiclaw/workspace/skills`（ワークスペース）
2. `~/.ebiclaw/skills`（グローバル）
...
EbiClaw はデフォルトでサンドボックス環境で実行されます。Agent は設定されたワークスペース内のファイルアクセスとコマンド実行のみが可能です。
...
| オプション              | デフォルト値            | 説明                                  |
| ----------------------- | ----------------------- | ------------------------------------- |
| `workspace`             | `~/.ebiclaw/workspace` | Agent の作業ディレクトリ              |
...
exec セキュリティガードは EbiClaw が直接起動するコマンドラインのみを検査します。`make`、`go run`、`cargo`、`npm run`、またはカスタムビルドスクリプトなどの開発ツールが生成する子プロセスは再帰的に検査しません。
...
* ビルトインガードより強力な分離が必要な場合は、コンテナまたは VM 内で EbiClaw を実行してください。
...
EbiClaw は定期タスクを自動実行できます。ワークスペースに `HEARTBEAT.md` ファイルを作成してください：
...
- `EBICLAW_HEARTBEAT_ENABLED=false` で無効化
- `EBICLAW_HEARTBEAT_INTERVAL=60` で間隔を変更
...
| `volcengine` | LLM（Volcengine 直接）                  | [volcengine.com](https://www.volcengine.com/activity/codingplan?utm_campaign=EbiClaw&utm_content=EbiClaw&utm_medium=devrel&utm_source=OWO&utm_term=EbiClaw) |
...
> **新機能：** EbiClaw は**モデル中心**の設定アプローチを採用しました。`vendor/model` 形式（例：`zhipu/glm-4.7`）を指定するだけで新しい Provider を追加できます — **コード変更不要！**
...
| **VolcEngine (Doubao)** | `volcengine/`          | `https://ark.cn-beijing.volces.com/api/v3`          | OpenAI     | [取得](https://www.volcengine.com/activity/codingplan?utm_campaign=EbiClaw&utm_content=EbiClaw&utm_medium=devrel&utm_source=OWO&utm_term=EbiClaw) |
| **Antigravity**         | `antigravity/`         | Google Cloud                                        | Custom     | OAuth のみ                                                       |

#### ロードバランシング

同じモデル名に複数のエンドポイントを設定すると、EbiClaw が自動的にラウンドロビンします：
...
EbiClaw はプロトコルファミリーで Provider をルーティングします：
...
EbiClaw は `cron` ツールを通じて cron スタイルのスケジュールタスクをサポートします。
...
スケジュールタスクは再起動後も `~/.ebiclaw/workspace/cron/` に保存されます。
