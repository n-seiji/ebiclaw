...
EbiClaw performs multiple complex interactions under the hood for every single request it receives—from routing messages and evaluating complexity, to executing tools and adapting to model failures. Being able to see exactly what is happening is crucial, not just for troubleshooting potential issues, but also for truly understanding how the agent operates.
## Starting EbiClaw in Debug Mode

To get detailed information about what the agent is doing (LLM requests, tool calls, message routing), you can start the EbiClaw gateway with the debug flag:

```bash
ebiclaw gateway --debug
# or
ebiclaw gateway -d
```
...
By default, EbiClaw truncates very long strings (such as the *System Prompt* or large JSON output results) in the debug logs to keep the console readable.
...
```bash
ebiclaw gateway --debug --no-truncate
...
[...] [INFO] agent: Tool call: web_search({"query":"ebiclaw release notes"}) {tool=web_search, iteration=1}
```
...
```bash
EBICLAW_AGENTS_DEFAULTS_TOOL_FEEDBACK_ENABLED=true
EBICLAW_AGENTS_DEFAULTS_TOOL_FEEDBACK_MAX_ARGS_LENGTH=300
```
