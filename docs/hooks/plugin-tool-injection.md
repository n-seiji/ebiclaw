...
This document demonstrates how to use EbiClaw's hook system to implement external plugin tool injection, allowing LLM to call tools implemented by external hook processes.
...
This way, external hooks can fully implement plugin tools without registering any tools inside EbiClaw.
...
1. EbiClaw sends `hook.before_llm`, hook injects `get_weather` tool definition
2. LLM sees tool definition, decides to call `get_weather(city="Beijing")`
3. EbiClaw sends `hook.before_tool`, hook uses `respond` action to return weather data
...
These references are managed by EbiClaw's MediaStore and can be:
...
Injected plugin tools coexist with EbiClaw built-in tools:
...
3. **Coexist with built-in tools**: Does not affect normal operation of EbiClaw's original tools
