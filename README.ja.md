<div align="center">
  ...
  <h1>EbiClaw: Go で書かれた超効率 AI アシスタント</h1>

  <h3>$10 ハードウェア · 10MB RAM · ms 起動 · Let's Go, EbiClaw!</h3>
  <p>
...
> **EbiClaw** は [Sipeed](https://sipeed.com) が立ち上げた独立したオープンソースプロジェクトです。完全に **Go 言語**で一から書かれており、OpenClaw、NanoBot、その他のプロジェクトのフォークではありません。

**EbiClaw** は [NanoBot](https://github.com/HKUDS/nanobot) にインスパイアされた超軽量パーソナル AI アシスタントです。**Go** でゼロからリビルドされ、「セルフブートストラッピング」プロセスで構築されました — AI Agent 自身がアーキテクチャの移行とコード最適化を推進しました。
...
> **セキュリティに関する注意**
>
> * **暗号通貨なし:** EbiClaw には公式トークン/コインは**一切ありません**。`pump.fun` やその他の取引プラットフォームでの主張はすべて**詐欺**です。
...
> * **注記:** EbiClaw は初期開発段階にあり、未解決のネットワークセキュリティ問題がある可能性があります。v1.0 リリース前に本番環境へのデプロイは避けてください。
> * **注記:** EbiClaw は最近多くの PR をマージしており、最新バージョンではメモリフットプリントが大きくなる場合があります（10〜20MB）。機能セットが安定次第、リソース最適化を優先する予定です。

## 📢 ニュース

2026-03-31 📱 **Android サポート！** EbiClawがAndroidで動作！APKは[ebiclaw.io](https://ebiclaw.io/download)からダウンロード

2026-03-25 🚀 **v0.2.4 リリース！** Agent アーキテクチャ全面刷新（SubTurn、Hooks、Steering、EventBus）、WeChat/WeCom 統合、セキュリティ強化（.security.yml、機密データフィルタリング）、新プロバイダー（AWS Bedrock、Azure、Xiaomi MiMo）、35 件のバグ修正。EbiClaw **26K ⭐** 達成！

2026-03-17 🚀 **v0.2.3 リリース！** システムトレイ UI（Windows & Linux）、サブエージェントステータス追跡（`spawn_status`）、実験的 Gateway ホットリロード、cron セキュリティゲート、セキュリティ修正 2 件。EbiClaw **25K ⭐** 達成！
...
2026-02-26 🎉 EbiClaw がわずか 17 日で **20K スター** 達成！Channel 自動オーケストレーションとケイパビリティインターフェースが実装されました。

2026-02-16 🎉 EbiClaw が 1 週間で 12K スター達成！コミュニティメンテナーの役割と[ロードマップ](ROADMAP.md)が正式に公開されました。

2026-02-13 🎉 EbiClaw が 4 日間で 5000 スター達成！プロジェクトロードマップと開発者グループの準備が進行中。

2026-02-09 🎉 **EbiClaw リリース！** $10 ハードウェアで 10MB 未満の RAM で動く AI Agent を 1 日で構築。Let's Go, EbiClaw!
...
<img src="assets/compare.jpg" alt="EbiClaw" width="512">
...
<p align="center">
<img src="assets/hardware-banner.jpg" alt="EbiClaw Hardware Compatibility" width="100%">
</p>
...
EbiClaw はほぼすべての Linux デバイスにデプロイできます！
...
10 年前のスマホに第二の人生を！EbiClaw でスマート AI アシスタントに変身させましょう。
...
<img src="assets/termux.jpg" alt="EbiClaw on Termux" width="512">
...
EbiClaw は `model_list` 設定を通じて多数の LLM Provider をサポートしています。`protocol/model` 形式を使用してください：
...
メッセージングプラットフォームで EbiClaw と会話できます：
...
EbiClaw は最新情報を提供するために Web を検索できます。`tools.web` で設定してください：
...
EbiClaw にはファイル操作、コード実行、スケジューリングなどの組み込みツールが含まれています。詳細は [ツール設定](docs/ja/tools_configuration.md) を参照してください。
...
EbiClaw は [MCP](https://modelcontextprotocol.io/) をネイティブサポートしています — 任意の MCP サーバーに接続して、外部ツールやデータソースで Agent の機能を拡張できます。
...
CLI または統合チャットアプリからメッセージを 1 つ送るだけで、EbiClaw をエージェントソーシャルネットワークに接続できます。
...
EbiClaw は `cron` ツールによるスケジュールリマインダーと定期タスクをサポートしています：
...
[コミュニティロードマップ](https://github.com/n-seiji/ebiclaw/issues/988)と[CONTRIBUTING.md](CONTRIBUTING.md)をご覧ください。
