...
func TestLoadOpenClawConfig(t *testing.T) {
	...
	if workspace != "~/.ebiclaw/workspace" {
		t.Errorf("expected workspace '~/.ebiclaw/workspace', got '%s'", workspace)
	}
...
func TestConvertToEbiClaw(t *testing.T) {
	tmpDir := t.TempDir()
	configPath := filepath.Join(tmpDir, "openclaw.json")
	...
	picoCfg, warnings, err := cfg.ConvertToEbiClaw("")
	if err != nil {
	...
	if picoCfg.Agents.Defaults.Workspace != "~/.ebiclaw/workspace" {
		t.Errorf("expected workspace '~/.ebiclaw/workspace', got '%s'", picoCfg.Agents.Defaults.Workspace)
	}
...
func TestConvertToEbiClawWithQQAndDingTalk(t *testing.T) {
	tmpDir := t.TempDir()
	configPath := filepath.Join(tmpDir, "openclaw.json")
	...
	picoCfg, _, err := cfg.ConvertToEbiClaw("")
	if err != nil {
		t.Fatalf("failed to convert config: %v", err)
	}
...
func TestConvertToEbiClawWithMatrix(t *testing.T) {
	tmpDir := t.TempDir()
	configPath := filepath.Join(tmpDir, "openclaw.json")
...
func TestConvertToEbiClawWithMatrixDisabled(t *testing.T) {
	tmpDir := t.TempDir()
	configPath := filepath.Join(tmpDir, "openclaw.json")
...
func TestToStandardConfig(t *testing.T) {
	...
	if stdCfg.Agents.Defaults.Workspace != "~/.ebiclaw/workspace" {
		t.Errorf("expected workspace '~/.ebiclaw/workspace', got '%s'", stdCfg.Agents.Defaults.Workspace)
	}
