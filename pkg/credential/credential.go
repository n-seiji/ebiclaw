...
//   - Encrypted:   "enc://<base64>"     → AES-256-GCM decrypt via EBICLAW_KEY_PASSPHRASE
...
//  2. EBICLAW_SSH_KEY_PATH env var
//  3. ~/.ssh/ebiclaw_ed25519.key (os.UserHomeDir is cross-platform)
package credential
...
const PassphraseEnvVar = "EBICLAW_KEY_PASSPHRASE"

// PassphraseProvider is the function used to retrieve the passphrase for enc://
// credential decryption. It defaults to reading EBICLAW_KEY_PASSPHRASE from the
...
const SSHKeyPathEnvVar = "EBICLAW_SSH_KEY_PATH"

// ebiclawHome is a package-local copy of config.EnvHome. It is kept here to
// avoid a circular import between pkg/credential and pkg/config.
const ebiclawHome = "EBICLAW_HOME"

const (
	FileScheme = "file://"
	EncScheme  = "enc://"

	hkdfInfo = "ebiclaw-credential-v1"
	saltLen  = 16
	nonceLen = 12
	keyLen   = 32
)
...
//
// passphrase is required (EBICLAW_KEY_PASSPHRASE value).
// sshKeyPath is the SSH private key file to use; pass "" to auto-detect via
// EBICLAW_SSH_KEY_PATH env var or ~/.ssh/ebiclaw_ed25519.key.
...
//   - exact match with EBICLAW_SSH_KEY_PATH env var
//   - within the EBICLAW_HOME env var directory
//   - within ~/.ssh/
func allowedSSHKeyPath(path string) bool {
	...
	// Exact match with EBICLAW_SSH_KEY_PATH.
	...
	// Within EBICLAW_HOME.
	if picoHome := os.Getenv(ebiclawHome); picoHome != "" {
...
// Final key: HKDF-SHA256(ikm, salt, info="ebiclaw-credential-v1", 32 bytes)
	...
	if sshKeyPath == "" {
		return nil, fmt.Errorf(
			"credential: SSH private key is required but not found" +
				" (set EBICLAW_SSH_KEY_PATH or place key at ~/.ssh/ebiclaw_ed25519.key)")
	}
	if !allowedSSHKeyPath(sshKeyPath) {
		return nil, fmt.Errorf(
			"credential: SSH key path %q is not in an allowed location (EBICLAW_SSH_KEY_PATH, EBICLAW_HOME, or ~/.ssh/)",
			sshKeyPath,
		)
	}
...
//  2. EBICLAW_SSH_KEY_PATH env var
//  3. ~/.ssh/ebiclaw_ed25519.key (auto-detection)
//
...
// findDefaultSSHKey returns the ebiclaw-specific SSH key path if it exists.
