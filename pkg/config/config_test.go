...
// EBICLAW_SSH_KEY_PATH to its path for the duration of the test. This is required
// whenever a test exercises encryption/decryption via credential.Encrypt or SaveConfig.
func mustSetupSSHKey(t *testing.T) {
	t.Helper()
	keyPath := filepath.Join(t.TempDir(), "ebiclaw_ed25519.key")
	if err := credential.GenerateSSHKey(keyPath); err != nil {
		t.Fatalf("mustSetupSSHKey: %v", err)
	}
	t.Setenv("EBICLAW_SSH_KEY_PATH", keyPath)
}
  ...
  "hooks": {
    "processes": {
      "review-gate": {
        "enabled": true,
        "transport": "stdio",
        "command": ["uvx", "ebiclaw-hook-reviewer"],
        "dir": "/tmp/hooks",
        "env": {
          "HOOK_MODE": "rewrite"
        },
...
func TestDefaultConfig_WorkspacePath_Default(t *testing.T) {
	...
	cfg := DefaultConfig()
	want := filepath.Join(fakeHome, ".ebiclaw", "workspace")
...
func TestDefaultConfig_WorkspacePath_WithEbiclawHome(t *testing.T) {
	t.Setenv("EBICLAW_HOME", "/custom/ebiclaw/home")

	cfg := DefaultConfig()
	want := filepath.Join("/custom/ebiclaw/home", "workspace")

	if cfg.Agents.Defaults.Workspace != want {
		t.Errorf("Workspace path with EBICLAW_HOME = %q, want %q", cfg.Agents.Defaults.Workspace, want)
	}
}
...
`
	...
	t.Setenv("EBICLAW_KEY_PASSPHRASE", "test-passphrase")
	t.Setenv("EBICLAW_SSH_KEY_PATH", "")
...
// unchanged when EBICLAW_KEY_PASSPHRASE is not set.
func TestLoadConfig_NoSealWithoutPassphrase(t *testing.T) {
	...
	t.Setenv("EBICLAW_KEY_PASSPHRASE", "")
	t.Setenv("EBICLAW_SSH_KEY_PATH", "")
...
// TestLoadConfig_MixedKeys_NoPassphrase verifies that when EBICLAW_KEY_PASSPHRASE
...
func TestCodexPipeConfigParse(t *testing.T) {
	raw := `{"codex_pipe":{"model":"gpt-5-codex","workspace":"/tmp/ws","sandbox":"danger-full-access"}}`
	var cfg Config
