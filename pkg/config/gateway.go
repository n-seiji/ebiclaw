...
type GatewayConfig struct {
	Host      string `json:"host"                env:"EBICLAW_GATEWAY_HOST"`
	Port      int    `json:"port"                env:"EBICLAW_GATEWAY_PORT"`
	HotReload bool   `json:"hot_reload"          env:"EBICLAW_GATEWAY_HOT_RELOAD"`
	LogLevel  string `json:"log_level,omitempty" env:"EBICLAW_LOG_LEVEL"`
}
...
// The EBICLAW_LOG_LEVEL environment variable overrides the file value.
func ResolveGatewayLogLevel(path string) string {
	...
	if envLevel := os.Getenv("EBICLAW_LOG_LEVEL"); envLevel != "" {
