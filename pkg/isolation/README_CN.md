...
`pkg/isolation` 为 `ebiclaw` 启动的子进程提供进程级隔离能力。

它当前不会把 `ebiclaw` 主进程自身放进沙箱中运行。
...
- 如果设置了 `EBICLAW_HOME`，使用该值。
- 否则默认使用用户目录下的 `.ebiclaw`。
...
隔离开启后，EbiClaw 会打印生成后的隔离计划，便于排障。
