...
const (
	autoStartEntryName = "EbiClawLauncher"
	launchAgentLabel   = "io.ebiclaw.launcher"
)
...
func linuxAutoStartPath() string {
	home, _ := os.UserHomeDir()
	return filepath.Join(home, ".config", "autostart", "ebiclaw-web.desktop")
}
...
func setLinuxAutoStart(enabled bool, exePath string, args []string) error {
	...
	if enabled {
		...
		content := strings.Join([]string{
			...
			"Version=1.0",
			"Name=EbiClaw Web",
			"Comment=Start EbiClaw Web on login",
