...
EBICLAW_HOME?=$(HOME)/.ebiclaw
WORKSPACE_DIR?=$(EBICLAW_HOME)/workspace
...
## build: Build the ebiclaw binary for current platform
build: generate
...
## build-launcher: Build the ebiclaw-launcher (web console) binary
build-launcher:
	@echo "Building ebiclaw-launcher for $(PLATFORM)/$(ARCH)..."
	@mkdir -p $(BUILD_DIR)
	@GOARCH=${ARCH} $(MAKE) -C web build \
		OUTPUT="$(CURDIR)/$(BUILD_DIR)/ebiclaw-launcher-$(PLATFORM)-$(ARCH)$(EXT)" \
		WEB_GO='$(WEB_GO)' \
...
## build-launcher-tui: Build the ebiclaw-launcher TUI binary
build-launcher-tui:
	@echo "Building ebiclaw-launcher-tui for $(PLATFORM)/$(ARCH)..."
	@mkdir -p $(BUILD_DIR)
	@$(GO) build $(GOFLAGS) -o $(BUILD_DIR)/ebiclaw-launcher-tui-$(PLATFORM)-$(ARCH) ./cmd/ebiclaw-launcher-tui
...
## build-all: Build ebiclaw for all platforms
build-all: generate
	@echo "Building for multiple platforms..."
	@mkdir -p $(BUILD_DIR)
...
## install: Install ebiclaw to system and copy builtin skills
install: build
...
## uninstall: Remove ebiclaw from system
uninstall:
...
## uninstall-all: Remove ebiclaw and all data
uninstall-all:
...
vet: generate
	@packages="$$($(GO) list $(GOFLAGS) ./...)" && \
		$(GO) vet $(GOFLAGS) $$(printf '%s\n' "$$packages" | grep -v '^github.com/n-seiji/ebiclaw/web/')
	@cd web/backend && $(WEB_GO) vet ./...

## test: Test Go code
test: generate
	@$(GO) test $(GOFLAGS) $$($(GO) list $(GOFLAGS) ./... | grep -v github.com/n-seiji/ebiclaw/web/)
	@cd web && make test
...
## docker-run: Run ebiclaw gateway in Docker (Alpine-based)
docker-run:
	docker compose -f docker/docker-compose.yml --profile gateway up

## docker-run-full: Run ebiclaw gateway in Docker (full-featured)
docker-run-full:
	docker compose -f docker/docker-compose.full.yml --profile gateway up

## docker-run-agent: Run ebiclaw agent in Docker (interactive, Alpine-based)
docker-run-agent:
	docker compose -f docker/docker-compose.yml run --rm ebiclaw-agent

## docker-run-agent-full: Run ebiclaw agent in Docker (interactive, full-featured)
docker-run-agent-full:
	docker compose -f docker/docker-compose.full.yml run --rm ebiclaw-agent
...
## build-macos-app: Build EbiClaw macOS .app bundle (no terminal window)
...
help:
	...
	@echo "  WORKSPACE_DIR           # Workspace directory (default: ~/.ebiclaw/workspace)"
